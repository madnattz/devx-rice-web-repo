...
Public Class ctlSimsReport12
    ...
    Public Sub LoadData()
        Try
            ...
            Dim cmd As String = "SP_Sims_Report12_SaleSeedByPlaces"
            ...
            End If
            prm(2) = New SqlClient.SqlParameter("@SeedStatus", DbType.String)
            If cmbSeedStatus.SelectedIndex <> 0 Then
                prm(2).Value = cmbSeedStatus.Text
            Else
                prm(2).Value = DBNull.Value
            End If
            prm(3) = New SqlClient.SqlParameter("@SeedType", DbType.String)
            If cmbSeedType.SelectedIndex <> 0 Then
                prm(3).Value = cmbSeedType.Text
            Else
                prm(3).Value = DBNull.Value
            End If
            prm(4) = New SqlClient.SqlParameter("@SeedClass", DbType.String)
            If cmbSeedClass.SelectedIndex <> 0 Then
                prm(4).Value = cmbSeedClass.Text
            Else
                prm(4).Value = DBNull.Value
            End If
            prm(5) = New SqlClient.SqlParameter("@Season", DbType.String)
            If cmbSeason.SelectedIndex <> 0 Then
                prm(5).Value = cmbSeason.Text
            Else
                prm(5).Value = DBNull.Value
            End If
            prm(6) = New SqlClient.SqlParameter("@SeedYear", DbType.String)
            If cmbYear.SelectedIndex <> 0 Then
                prm(6).Value = cmbYear.Text
            Else
                prm(6).Value = DBNull.Value
            End If
            prm(7) = New SqlClient.SqlParameter("@MoneyType", DbType.String)
            If cmbMoneyType.SelectedIndex <> 0 Then
                prm(7).Value = cmbMoneyType.Text
            Else
                prm(7).Value = DBNull.Value
            End If
            prm(8) = New SqlClient.SqlParameter("@LotNo", DbType.Int32)
            If txtLotNo.Text <> "000" And Not txtLotNo.Text.ToLower.Contains("x") Then
                prm(8).Value = Convert.ToInt32(txtLotNo.Text)
            Else
                prm(8).Value = DBNull.Value
            End If
            prm(9) = New SqlClient.SqlParameter("@StartDate", DbType.Date)
            If dpkStartDate.Text <> "" Then
                prm(9).Value = dpkStartDate.Value
            Else
                prm(9).Value = DBNull.Value
            End If
            prm(10) = New SqlClient.SqlParameter("@EndDate", DbType.Date)
            If dpkEndDate.Text <> "" Then
                prm(10).Value = dpkEndDate.Value
            Else
    ...
    Protected Sub btnExportData_Click(sender As Object, e As EventArgs) Handles btnExportData.Click
        LoadData()
        ASPxGridViewExporter1.WriteXlsToResponse(Date.Today.ToString("ddMMyy") & "_รายงานการจำหน่ายเมล็ดพันธุ์ในพื้นที่ต่างๆ", True)
    End Sub
