...
Public Class ctlSimsReport15
    ...
    Public Sub LoadData()
        Try
            ...
            Dim cmd As String = "SP_Sims_Report15_GetAgents"
            ...
            End If
            prm(4) = New SqlClient.SqlParameter("@AgentStartDate", DbType.Date)
            If dpkApproveStartDate.Text <> "" Then
                prm(4).Value = dpkApproveStartDate.Value
            Else
                prm(4).Value = DBNull.Value
            End If
            prm(5) = New SqlClient.SqlParameter("@AgentEndDate", DbType.Date)
            If dpkApproveEndDate.Text <> "" Then
                prm(5).Value = dpkApproveEndDate.Value
            Else
                prm(5).Value = DBNull.Value
            End If
            prm(6) = New SqlClient.SqlParameter("@AgentExpireDate", DbType.Date)
            If dpkExpireStartDate.Text <> "" Then
                prm(6).Value = dpkExpireStartDate.Value
            Else
                prm(6).Value = DBNull.Value
            End If
            prm(7) = New SqlClient.SqlParameter("@AgentEndExpireDate", DbType.Date)
            If dpkExpireEndDate.Text <> "" Then
                prm(7).Value = dpkExpireEndDate.Value
            Else
    ...
    Protected Sub btnExportData_Click(sender As Object, e As EventArgs) Handles btnExportData.Click
        LoadData()
        ASPxGridViewExporter1.WriteXlsToResponse(Date.Today.ToString("ddMMyy") & "_ทะเบียนรายชื่อตัวแทนจำหน่ายเมล็ดพันธุ์พืช", True)
    End Sub
