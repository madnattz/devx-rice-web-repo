...
Public Class ctlSimsReport13
    ...
    Public Sub LoadData()
        Try
            ...
            Dim cmd As String = "SP_Sims_Report13_TransferSeed"
            Dim prm(11) As SqlClient.SqlParameter

            prm(0) = New SqlClient.SqlParameter("@SiteNo", DbType.Int32)
            If cmbTransferSite.SelectedIndex <> 0 Then
                prm(0).Value = cmbTransferSite.Value
            Else
                prm(0).Value = DBNull.Value
            End If
            prm(1) = New SqlClient.SqlParameter("@TransferReason", DbType.Int32)
            If cmbTransferReason.SelectedIndex <> 0 Then
                prm(1).Value = cmbTransferReason.Value
            Else
                prm(1).Value = DBNull.Value
            End If
            prm(2) = New SqlClient.SqlParameter("@PlantName", DbType.String)
            If cmbPlant.SelectedIndex <> 0 Then
                prm(2).Value = cmbPlant.Text
            Else
                prm(2).Value = DBNull.Value
            End If
            prm(3) = New SqlClient.SqlParameter("@SeedStatus", DbType.String)
            If cmbSeedStatus.SelectedIndex <> 0 Then
                prm(3).Value = cmbSeedStatus.Text
            Else
                prm(3).Value = DBNull.Value
            End If
            prm(4) = New SqlClient.SqlParameter("@SeedType", DbType.String)
            If cmbSeedType.SelectedIndex <> 0 Then
                prm(4).Value = cmbSeedType.Text
            Else
                prm(4).Value = DBNull.Value
            End If
            prm(5) = New SqlClient.SqlParameter("@SeedClass", DbType.String)
            If cmbSeedClass.SelectedIndex <> 0 Then
                prm(5).Value = cmbSeedClass.Text
            Else
                prm(5).Value = DBNull.Value
            End If
            prm(6) = New SqlClient.SqlParameter("@Season", DbType.String)
            If cmbSeason.SelectedIndex <> 0 Then
                prm(6).Value = cmbSeason.Text
            Else
                prm(6).Value = DBNull.Value
            End If
            prm(7) = New SqlClient.SqlParameter("@SeedYear", DbType.String)
            If cmbYear.SelectedIndex <> 0 Then
                prm(7).Value = cmbYear.Text
            Else
                prm(7).Value = DBNull.Value
            End If
            prm(8) = New SqlClient.SqlParameter("@MoneyType", DbType.String)
            If cmbMoneyType.SelectedIndex <> 0 Then
                prm(8).Value = cmbMoneyType.Text
            Else
                prm(8).Value = DBNull.Value
            End If
            prm(9) = New SqlClient.SqlParameter("@LotNo", DbType.Int32)
            If txtLotNo.Text <> "000" And Not txtLotNo.Text.ToLower.Contains("x") Then
                prm(9).Value = Convert.ToInt32(txtLotNo.Text)
            Else
                prm(9).Value = DBNull.Value
            End If
            prm(10) = New SqlClient.SqlParameter("@StartDate", DbType.Date)
            If dpkStartDate.Text <> "" Then
                prm(10).Value = dpkStartDate.Value
            Else
                prm(10).Value = DBNull.Value
            End If
            prm(11) = New SqlClient.SqlParameter("@EndDate", DbType.Date)
            If dpkEndDate.Text <> "" Then
                prm(11).Value = dpkEndDate.Value
            Else
    ...
    Protected Sub btnExportData_Click(sender As Object, e As EventArgs) Handles btnExportData.Click
        LoadData()
        ASPxGridViewExporter1.WriteXlsToResponse(Date.Today.ToString("ddMMyy") & "_รายงานการโอนเมล็ดพันธุ์", True)
    End Sub
