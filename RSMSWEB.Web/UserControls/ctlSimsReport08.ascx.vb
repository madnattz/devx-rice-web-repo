...
Public Class ctlSimsReport08
    ...
    Public Sub LoadData()
        Try
            ...
            Dim cmd As String = "SP_Sims_Report08_MarketSituation"
            ...
            'End If
            prm(6) = New SqlClient.SqlParameter("@StartDate", DbType.Date)
            If dpkStartDate.Text <> "" Then
                prm(6).Value = dpkStartDate.Value
            Else
                prm(6).Value = DBNull.Value
            End If
            prm(7) = New SqlClient.SqlParameter("@EndDate", DbType.Date)
            If dpkEndDate.Text <> "" Then
                prm(7).Value = dpkEndDate.Value
            Else
                prm(7).Value = DBNull.Value
            End If
    ...
    Protected Sub btnExportData_Click(sender As Object, e As EventArgs) Handles btnExportData.Click
        LoadData()
        ASPxGridViewExporter1.WriteXlsToResponse(Date.Today.ToString("ddMMyy") & "_รายงานสถานการณ์ตลาดและคลังเมล็ดพันธุ์", True)
    End Sub
